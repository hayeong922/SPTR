...
class IndustryTermRecogniser(object):
    ...
    def __init__(self, solr_url):
        ...
        try:
            ...
            self._logger.exception("Oops! 'solr_field_term_candidates' is not found in config file. Default to [%s]",
                                   FIELD_TERM_CANDIDATES)

        global FIELD_CONTENT
        try:
            FIELD_CONTENT = config['DEFAULT']['solr_field_content']
        except KeyError:
            self._logger.exception("Oops! 'solr_field_content' is not found in config file. Default to [%s]",
                                   FIELD_CONTENT)
        ...
        try:
            ...
            self._logger.exception(
                "Oops! 'index_dict_term_with_industry_term' is set incorrectly in config file. Default to set true")
            self.index_dict_term_with_industry_term = True
        ...
        try:
            ...
            self._logger.exception("Oops! 'solr_field_industry_term' is not found in config file. Default to [%s]",
                                   FIELD_TERM_CANDIDATES)
        ...
        try:
            ...
            if "true" == self.export_term_candidates.lower():
                ...
                raise Exception(
                    "current setting [%s] for 'export_term_candidates' is not supported!" % self.export_term_variants)
        except KeyError:
            self._logger.exception(
                "Oops! 'export_term_candidates' is set incorrectly in config file. Default to set false")
            self.export_term_candidates = False
        ...
        try:
            ...
            if "true" == self.export_term_variants.lower():
                ...
                raise Exception(
                    "current setting [%s] for 'export_term_variants' is not supported!" % self.export_term_variants)
        except KeyError:
            self._logger.exception(
                "Oops! 'export_term_variants' is set incorrectly in config file. Default to set false")
            self.export_term_variants = False
        ...
        try:
            ...
            self._logger.exception(
                "Oops! 'term_variants_export_file_name' is set incorrectly in config file. Default to set 'term_variants'")
        ...
        try:
            ...
            self._logger.exception(
                "Oops! 'solr_field_dictionary_term' is set incorrectly in config file. Default to index into the 'dictTerm_ss' field")
            FIELD_DICTIONARY_TERM = 'dictTerm_ss'
    ...
    def terminology_tagging(self):
        ...
        final_term_set = [term_tuple[0] for term_tuple in ranked_term_tuple_list if
                          term_tuple[1] > self.cut_off_threshold]
    ...
    def synonym_aggregation(self, terms=set()):
        ...
        export_list_of_list_to_csv(os.path.join(os.path.dirname(__file__), '..'), self.term_variants_export_file_name,
                                   list(aggregated_terms))
...
class TermRanker(object):
    ...
    def __init__(self, solr_client):
        ...
        try:
            ...
            self._logger.exception(
                "Oops! 'solr_field_doc_id' is not found in config file. Default to 'id' field instead.")
...
class CValueRanker(TermRanker):
    ...
    def ranking(self):
        ...
        if all_candidates is None or len(all_candidates) == 0:
            self._logger.info("No candidate found. Skip ranking.");
            return [];
        ...
        with MultiprocPool(processes=int(self.parallel_workers)) as pool:
            ...
            ranked_all_candidates = pool.starmap(term_weight_async_calculation,
                                                 [(self.solrClient.solrURL, candidate, optional_parameter) for candidate
                                                  in all_candidates])

        self._logger.info(" all candidates c-value computation is completed.")

        self._logger.info(" all candidates c-value ranking...")
        ranked_all_candidates = {term_cvalue_tuple[0]: term_cvalue_tuple[1] for term_cvalue_tuple in
